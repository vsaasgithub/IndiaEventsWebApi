...
namespace IndiaEventsWebApi.Controllers
{
    ...
    public class WebHooksController : ControllerBase
    {
        ...
        //[HttpPost]
        //public async Task<IActionResult> WebhookHandler([FromBody] SmartsheetWebhookPayload payload)
        //{
        //    try
        //    {

        //        string eventId = payload?.Event?.ObjectId.ToString();
        //        string status = payload?.Changes?.FirstOrDefault()?.Cell?.Value?.ToString();


        //        if (!string.IsNullOrEmpty(eventId) && !string.IsNullOrEmpty(status) && status.Equals("Approved", StringComparison.OrdinalIgnoreCase))
        //        {
